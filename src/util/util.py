...
class csvHelper():
    ...
    def csvRead(self):
        try:
            csvFile = csv.reader(open(self.csvPath, 'r', encoding="utf8"))
        except Exception as e:
            print("Error: %s,\t,%s" % (Exception, e))
            print("There may be some problem in %s" % self.csvPath)
            return ""
        return csvFile

    # csv读取（详细版）
    def csvReadDetail(self):
        try:
            csvFile = csv.reader(open(self.csvPath, 'r'))
        except Exception as e:
            print("Error: %s,\t,%s" % (Exception, e))
            print("There may be some problem in %s" % self.csvPath)
            return ""
